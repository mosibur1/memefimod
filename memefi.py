...
# HANDLE ALL ERRORS PUT THEM HERE BANG SAVE_POST
...
def Check_user(index):
    ...
    headers = headers_set.copy()  # Make a copy of headers_set so as not to change the global variable
...
def activate_energy_recharge_booster(index, headers):
    ...
    if response and 'data' in response and response['data'] and 'telegramGameActivateBooster' in response['data']:
        ...
        print(f"\n🔋 Energy filled. Current energy: {new_energy}")
    else:
        print("❌ Failed to activate Recharge Booster: Data is incomplete or missing.")

def activate_booster(index, headers):
    access_token = fetch(index + 1)
    url = "api-gw-tg.memefi.club"
    print("\r🚀 Enabling Turbo Boost ... ", end="", flush=True)
...
def check_and_complete_tasks(index, headers):
    ...
    if all_completed:
        print(f"\r[ Account {index + 1} ] All tasks have been completed. ✅            ", flush=True)
        return True
    ...
    for task in tasks['data']['campaignTasks']:
        ...
        if task['status'] == "Pending":
            ...
            print(f"\r🔍 Verification task: {task['name']}                                                                ", end="", flush=True)
            verify_task_payload = {
    ...
    # Check ulang task setelah memindahkan ke verification
    ...
    for task in updated_tasks['data']['campaignTasks']:
        ...
        if task['status'] == "Verification":
            print(f"\r🔥 Solve task: {task['name']}", end="", flush=True)
...
def main():
    ...
    print("\r Mendapatkan list Account valid...", end="", flush=True)
  
    while True:
        with open('query_id.txt', 'r') as file:
            lines = file.readlines()

        # Kumpulkan informasi Account terlebih dahulu
        accounts = []
        ...
        # Setelah menampilkan semua Account, mulai checking tugas
        for index, result, first_name, last_name, league in accounts:
            ...
            if stat_result is not None:
                ...
                if darah_bos == 0:
                    print("\nThe boss has been defeated, setting up the next boss...", flush=True)
                    set_next_boss(index, headers)
                ...
                if energy_sekarang < 0.25 * user_data['maxEnergy']:
                    if auto_booster == 'y':
                        if user_data['freeBoosts']['currentRefillEnergyAmount'] > 0:
                            print("\r🪫 Energy Out, activate Recharge Booster... \n", end="", flush=True)
                            activate_energy_recharge_booster(index, headers)
                            continue  # Lanjutkan tapping setelah recharge
                        else:
                            print("\r🪫 Energy out, no booster available. Switch to next Account.\n", flush=True)
                            continue  # Beralih ke Account berikutnya
                    else:
                        print("\r🪫 Energy Out, auto booster disable. Switch to next Account.\n", flush=True)
                        continue  # Beralih ke Account berikutnya
...
def animate_energy_recharge(duration):
    ...
    while time.time() < end_time:
        ...
        for frame in frames:
            print(f"\r🪫 Recharge energy {frame} - seconds {remaining_time} left         ", end="", flush=True)
            time.sleep(0.25)
